...
let ListProviderDayAvailabilityService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const eachHourArray = Array.from({
      length: 10
    }, (_, index) => index + hourStart); // Date.now() precisa ser executado antes do map pois o resultado está mockado uma vez no teste unitário.

    const currentDate = new Date(Date.now());
    const availability = eachHourArray.map(hour => {
      const hasAppointInHour = appointments.find(appointment => (0, _dateFns.getHours)(appointment.date) === hour);
      const compareDate = new Date(year, month - 1, day, hour);
      return {
        hour,
        availability: !hasAppointInHour && (0, _dateFns.isAfter)(compareDate, currentDate)
      };
    });
    return availability;
  }
